...
<Example class="bd-example-buttons justify-items-start" code={[...getData('theme-colors').map((themeColor) => `<button type="button" class="btn-solid btn-styled theme-${themeColor.name}">${themeColor.title}</button>
